...
static void string_mark_and_trace(gc_t *gc, string_t *str);
...
static void value_mark_and_trace(gc_t * gc, value_t *value) {
  if(value_is_pair(value)) {
    ...
    string_mark_and_trace(gc, value->data.as_string);
...
/** Traçage et marquage du contenu d'une chaine */
static void string_mark_and_trace(gc_t *gc, string_t *str) {
  if(str->gc_mark != gc->current_mark) {
    // la chaine n'est pas encore marquée
    if (gc->debug_gc) {
      printf("[GC]       ==> 1 string marked\n");
    }
    str->gc_mark = gc->current_mark;
  }
}
